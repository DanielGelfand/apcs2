// Team MazeRunners - Shayan Chowdhury, T. Fabiha, Oliver Frank, Daniel Gelfand
// APCS2 pd2
 ...
 * 1. Are you at the exit? If yes then you are done. If no then continue.
 * 2. Look to see if you can move to the next space in this order:
 *    up, right, down, left.
 * 3. If you can move that way, move. Else repeat step 2 until you don't
 *    have any options to move to.
 * 4. If you can't move in any direction, then you are done. Else, repeat
 *    from step 1.
 ***/
...
class MazeSolver
{
	...
	//recursive reduction
	    ...
	    if ( y - 1 >= 0 && (maze[x][y-1] == PATH || maze[x][y-1] == EXIT)) {
		solve(x,y-1);
	    }
	    if ( x + 1 < w && (maze[x+1][y] == PATH || maze[x+1][y] == EXIT)) {
		solve(x+1,y);
	    }
	    if ( y + 1 < h && (maze[x][y+1] == PATH || maze[x][y+1] == EXIT)) {
		solve(x,y+1);
	    }
	    if ( x - 1 >= 0 && (maze[x-1][y] == PATH || maze[x-1][y] == EXIT)) {
		solve(x-1,y);
	    }
	}
    }
